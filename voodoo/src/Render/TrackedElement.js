...
/**
 * Tracks the position and size of an HTML element and issues callbacks when these change.
 *
 * @constructor
 * @private
 *
...
function TrackedElement_(element) {
  log_.assert_(element, 'element must be valid.', '(TrackedElement_::TrackedElement_)');
 ...
 * @private
 *
 * @param {function(number, number, number, number, boolean, boolean)} callback Callback to add.
 * @return {number} Callback id used to release.
 */
TrackedElement_.prototype.addCallback_ = function(callback) {
  log_.assert_(callback, 'callback must be valid.', '(TrackedElement_::addCallback_)');
  ...
  // Always call the callback once immediately after it is added so that the new object attaching
  // gets the current location of the element.
  callback(this.lastX_, this.lastY_, this.lastWidth_, this.lastHeight_, true, true);
...
TrackedElement_.prototype.releaseCallback_ = function(callbackId) {
  log_.assert_(callbackId >= 0, 'callbackId must be >= 0.', callbackId,
      '(TrackedElement_::releaseCallback_)');
  log_.assert_(typeof callbackId === 'number', 'callbackId must be a number.', callbackId,
      '(TrackedElement_::releaseCallback_)');
...
/**
 * Called each frame to detect changes in position or size of HTML elements and fire the callbacks
 * to update local coordinate systems.
 *
 * @private
 */
TrackedElement_.prototype.update_ = function() {
  var position = window['voodoo']['utility']['findAbsolutePosition'](this.element_);
  ...
  if (moved || resized) {
    ...
    for (var callback in this.callbacks_) {
      if (this.callbacks_.hasOwnProperty(callback)) {
        this.callbacks_[callback].call(null, this.lastX_, this.lastY_, this.lastWidth_,
            this.lastHeight_, moved, resized);
      }
    }
  }
};
